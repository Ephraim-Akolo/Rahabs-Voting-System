...
from website.models import PoliticalParty, State, LocalGorvernment, Election, User, Candidate

def insert_political_parties():
    # Create and save instances of YourModel
    data_to_insert = [ {"name": f"A{chr(i)}C"} for i in range(65, 91)
    ]
...
def insert_states_and_lga(max_lga=5, ignore_states=[]):
    ...
    data_to_insert = {
        ...
        "Borno": ["Abadam", "Askira/Uba", "Bama", "Bayo", "Chibok", "Damboa", "Dikwa", "Gubio", "Guzamala", "Gwoza", "Hawul", "Jere", "Kaga", "Kala/Balge", "Konduga", "Kukawa", "Kwaya Kusar", "Mafa", "Magumeri", "Maiduguri", "Marte", "Mobbar", "Monguno", "Ngala", "Nganzai", "Shani"],
        "Cross River": ["Akpabuyo", "Bakassi", "Bekwarra", "Biase", "Calabar Municipal", "Calabar South", "Etung", "Ikom", "Obanliku", "Obubra", "Obudu", "Odukpani", "Ogoja", "Yakurr", "Yala"],
        ...
        "Kwara": ["Asa", "Baruten", "Edu", "Ekiti", "Ifelodun", "Ilorin East", "Ilorin South", "Ilorin West", "Irepodun", "Isin", "Kaiama", "Moro", "Offa", "Oke Ero", "Oyun"],
    ...
    for state, lgas in data_to_insert.items():
        print(state)
        if state in ignore_states:
            continue
        st = State.objects.create(name=state)
        for lga in lgas[:max_lga]:
            LocalGorvernment.objects.create(state=st, name=lga)


def insert_political_candidates():
    # Create and save instances of YourModel
    apc = PoliticalParty.objects.get(name='APC')
    aac = PoliticalParty.objects.get(name='AAC')
    acc = PoliticalParty.objects.get(name='ACC')
    data_to_insert = [ 
        {"name": "Muhammadu Buhari", "party": apc},
        {"name": "Sani Abacha", "party": aac},
        {"name": "Olusegun Obasanjo", "party": acc},
    ]
...
if __name__ == "__main__":
    # insert_political_parties()
    # insert_states_and_lga(10, ("Abia", "Adamawa", "Akwa Ibom", "Anambra", "Bauchi", "Bayelsa", "Benue", "Borno", "Cross River", "Delta", "Ebonyi", "Edo", "Ekiti", "Enugu", "Gombe", "Imo", "Jigawa", "Kaduna", "Kano", "Katsina", "Kebbi", "Kogi", "Kwara", "Lagos", "Nasarawa", "Niger", "Ogun",))
    insert_political_candidates()
