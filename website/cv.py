...
def load_dir(path):
    return cv2.resize(cv2.cvtColor(cv2.imread(Path(path).as_posix()), cv2.COLOR_BGR2RGB), (500, 500))

def predict_img(img):
    _img = np.expand_dims(img, axis=0)
    assert resnet_model
    pred = resnet_model.predict(_img)
    return ['differentFaces', 'sameFaces'][np.argmax(pred)]
